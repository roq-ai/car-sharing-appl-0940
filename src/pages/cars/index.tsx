...
import { useRoqClient, useCarFindManyWithCount } from 'lib/roq';
...
export function CarListPage(props: CarListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'booking.count', 'review.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['model.contains', 'color.contains', 'license_plate.contains'],
          ...params.filters,
        },
        'car',
      ),
  ...
  const { data, error, isLoading, mutate } = useCarFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.car.delete({ where: { id } });
      await mutate();
    } catch (error) {
