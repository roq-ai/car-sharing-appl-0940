...
import { useRoqClient, useBookingFindManyWithCount } from 'lib/roq';
...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useBookingFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.booking.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.start_time ? format(record?.start_time, 'dd-MM-yyyy') : '',
    },
    {
      id: 'end_time',
      header: 'End Time',
      accessorKey: 'end_time',
      cell: ({ row: { original: record } }: any) => (record?.end_time ? format(record?.end_time, 'dd-MM-yyyy') : ''),
    },
