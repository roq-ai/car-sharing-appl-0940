...
import { useRoqClient, useReviewFindFirst } from 'lib/roq';
...
function ReviewEditPage() {
  ...
  const { data, error, isLoading, mutate } = useReviewFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ReviewInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.review.update({
        data: values as RoqTypes.review,
        where: {
          id,
        },
      });
