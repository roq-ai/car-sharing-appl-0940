...
import { useRoqClient, useReviewFindManyWithCount } from 'lib/roq';
...
export function ReviewListPage(props: ReviewListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useReviewFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.review.delete({ where: { id } });
      await mutate();
    } catch (error) {
